...
def validate(val_loader, model, args, device) -> float:
    ...
    progress = ProgressMeter(
        len(val_loader),
        [batch_time, losses1, losses2, top1, top2],
        prefix='Test: ')

    # switch to evaluate mode
    model.eval()
    if args.per_class_eval:
        confmat1 = ConfusionMatrix(len(args.class_names))
        confmat2 = ConfusionMatrix(len(args.class_names))
    else:
        confmat1 = None
        confmat2 = None
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(val_loader):
            ...
            cov_output = output[0]
            tra_output = output[1]
            cov_loss = F.cross_entropy(cov_output, target)
            tra_loss = F.cross_entropy(tra_output, target)
            # measure accuracy and record loss
            acc1, = accuracy(cov_output, target, topk=(1,))
            acc2, = accuracy(tra_output, target, topk=(1,))
            if confmat1 and confmat2:
                confmat1.update(target, cov_output.argmax(1))
                confmat2.update(target, tra_output.argmax(1))
            losses1.update(cov_loss.item(), images.size(0))
            losses2.update(tra_loss.item(), images.size(0))
        ...
        print(' * Acc@2 {top1.avg:.3f}'.format(top1=top2))
        if confmat1 and confmat2:
            print(confmat1.format(args.class_names))
            print(confmat2.format(args.class_names))

    return top1.avg, top2.avg


def validate_0(val_loader, model, args, device) -> float:
    batch_time = AverageMeter('Time', ':6.3f')
