...
from tllib.alignment.cdan import ConditionalDomainAdversarialLoss, ImageClassifier, ImageClassifier_v1
...
def main(args: argparse.Namespace):
    ...
    # wandb.init(project="cdan_v1", entity="1683016905", name=args.log.split('/')[-1])
    ...
    backbone = Conformer(patch_size=16, in_chans=3, num_classes=num_classes, base_channel=64, channel_ratio=1,
                           ...
                           qk_scale=None, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.).to(device)
    ...
    if args.randomized:
        ...
        domain_discri_1 = DomainDiscriminator(256 * num_classes, hidden_size=1024).to(device)
        # domain_discri_2 = DomainDiscriminator(384 * num_classes, hidden_size=1024).to(device)
        # domain_discri_1 = classifier.conv_cls_head
        # domain_discri_2 = classifier.trans_cls_head
    ...
    # optimizer = SGD(param_group, args.lr, momentum=args.momentum, weight_decay=args.weight_decay, nesterov=True)
    ...
    ad_optimizer = SGD(domain_discri_1.get_parameters(
    ...
    if args.phase == 'analysis':
        ...
        source_feature_c, source_feature_t = collect_feature(train_source_loader, classifier, device)
        target_feature_c, target_feature_t = collect_feature(train_target_loader, classifier, device)
        # plot t-SNE
        tSNE_filename_c = osp.join(logger.visualize_directory_c, 'TSNE_c.pdf')
        tSNE_filename_t = osp.join(logger.visualize_directory_t, 'TSNE_t.pdf')
        tsne.visualize(source_feature_c, target_feature_c, tSNE_filename_c)
        tsne.visualize(source_feature_t, target_feature_t, tSNE_filename_t)
        print("Saving t-SNE to", tSNE_filename_c)
        print("Saving t-SNE to", tSNE_filename_t)
        # calculate A-distance, which is a measure for distribution discrepancy
        A_distance_c = a_distance.calculate(source_feature_c, target_feature_c, device)
        A_distance_t = a_distance.calculate(source_feature_t, target_feature_t, device)
        print("A-distance =", A_distance_c)
        print("A-distance =", A_distance_t)
        return
    ...
    for epoch in range(args.epochs):
        ...
        print(lr_scheduler_ad.get_last_lr())
        print("lr:", lr_scheduler.get_last_lr()[0])
        # train for one epoch
        train(train_source_iter, train_target_iter, classifier, domain_adv_1, mcc_loss, optimizer, ad_optimizer,
              lr_scheduler, lr_scheduler_ad, epoch, args)
        # lr_scheduler.step()
        # evaluate on validation set
        acc1, acc2 = utils.validate(val_loader, classifier, args, device)
        # wandb.log({'epoch': epoch, 'val_acc1': acc1, 'val_acc2': acc2})
        acc = max(acc1, acc2)
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator, model: Conformer,
          domain_adv_1, mcc, optimizer, ad_optimizer, lr_scheduler, lr_scheduler_ad, epoch: int, args: argparse.Namespace):
    ...
    for i in range(args.iters_per_epoch):
        ...
        # loss = loss1+loss2
        loss = cov_cls_loss + tra_cls_loss
        ...
        # cov_transfer_loss = domain_adv_1(cov_s_cls, cov_s_fea, cov_t_cls, cov_t_fea) + mcc(cov_t_cls)
        # tra_transfer_loss = domain_adv_1(tra_s_cls, tra_s_fea, tra_t_cls, tra_t_fea) + mcc(tra_t_cls)
        ...
        # loss = cov_cls_loss + cov_transfer_loss
        ...
        # wandb.log({'iteration': epoch * args.iters_per_epoch + i, 'loss': losses.avg,
        #            'cls_acc_1': cls_accs1.avg, 'cls_acc_2': cls_accs2.avg})
...
if __name__ == '__main__':
    ...
    parser.add_argument("--phase", type=str, default='train', choices=['train', 'test', 'analysis', 'visual'],
