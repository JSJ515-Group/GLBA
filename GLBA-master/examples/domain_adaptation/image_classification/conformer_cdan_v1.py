...
class Attention(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4) #(3,8,6,197,64)
...
class Conformer(nn.Module):
    ...
    def __init__(self, patch_size=16, in_chans=3, num_classes=1000, base_channel=64, channel_ratio=4, num_med_block=0,
        ...
        # self.bottleneck_t = nn.Linear(embed_dim, 384)
        ...
        # self.bottleneck = nn.Linear(int(256 * channel_ratio), 384)
