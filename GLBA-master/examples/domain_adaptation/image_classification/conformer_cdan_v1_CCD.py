...
from timm.models.layers import DropPath, trunc_normal_, DropBlock2d
...
class ConvBlock(nn.Module):
    ...
    # def __init__(self, inplanes, outplanes, stride=1, res_conv=False, act_layer=nn.ReLU, groups=1,
    #              norm_layer=partial(nn.BatchNorm2d, eps=1e-6), drop_block=DropBlock2d(0.1, 7), drop_path=None):
...
class Conformer(nn.Module):
    ...
    def __init__(self, patch_size=16, in_chans=3, num_classes=1000, base_channel=64, channel_ratio=4, num_med_block=0,
        ...
        self.trans_cls_head = nn.Linear(1024, num_classes) if num_classes > 0 else nn.Identity()
    ...
    def forward(self, x):
        ...
        # if self.training:
        #     return [conv_cls, tran_cls], [x_p, x_t]
        # else:
    ...
    def get_bottleneck_t_parameters(self) -> List[Dict]:
        return [{"params": self.bottleneck_t.parameters(), "lr": 1.}]

    def get_tra_cls_parameters(self) -> List[Dict]:
        return [{"params": self.trans_cls_head.parameters(), "lr": 1.}]

    def get_bottleneck_parameters(self) -> List[Dict]:
        return [{"params": self.bottleneck.parameters(), "lr": 1.}]

    def get_cov_cls_parameters(self) -> List[Dict]:
        return [{"params": self.conv_cls_head.parameters(), "lr": 1.}]
