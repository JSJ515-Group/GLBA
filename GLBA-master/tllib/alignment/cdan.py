...
from tllib.modules.classifier import Classifier as Classifier
from tllib.modules.classifier import Classifier_v1 as Classifier_v1
from tllib.modules._classifier import _Classifier as _Classifier
...
class ImageClassifier_v1(Classifier_v1):
    def __init__(self, backbone: nn.Module, num_classes: int, embed_dim: Optional[int] = 384, channel_ratio=4):
        super(ImageClassifier_v1, self).__init__(backbone, num_classes, embed_dim, channel_ratio)


class _ImageClassifier(_Classifier):
    def __init__(self, backbone: nn.Module, bottleneck_dim: Optional[int] = 256, **kwargs):
        bottleneck = nn.Sequential(
            nn.Linear(backbone.out_features, bottleneck_dim),
            nn.BatchNorm1d(bottleneck_dim),
            nn.ReLU()
        )
        super(_ImageClassifier, self).__init__(backbone, bottleneck, bottleneck_dim, **kwargs)


class CT_Classifier(nn.Module):
    def __init__(self, cnn_classifier: nn.Module, vit_classifier: nn.Module, num_classes: int, bottleneck_dim: Optional[int] = 256):
    ...
    def forward(self, x: torch.Tensor):
        """"""
        x_p = self.cnn_classifier(x)
        x_t = self.vit_classifier(x)
        conv_cls = self.head(x_p)
    ...
    def get_parameters(self, base_lr=1.0):
        ...
        """
        params = [
            {"params": self.cnn_classifier.parameters(), "lr": 0.1 * base_lr},
            {"params": self.vit_classifier.parameters(), "lr": 0.1 * base_lr},
